...
import lk.ijse.gym.dao.Custom.Impl.EquipmentDaoImpl;
...
public class EquipmentFormController {
    ...
    private void loadAllEquipment() {
        var model = new EquipmentDaoImpl();

        ObservableList<EquipmentTm> obList = FXCollections.observableArrayList();
        try {
            List<EquipmentTm> dtoList = model.getAll();
            for (EquipmentTm  dto : dtoList) {
                obList.add(
                        new EquipmentTm(
                                dto.getEquipmentId(),
                                dto.getEquName(),
    ...
    void btnDeleteOnAction(ActionEvent event) {
        ...
        try {
            boolean isDeleted = equipmentBo.deleteEquipment(equipmentId);
            if (isDeleted) {
    ...
    void btnSaveOnAction(ActionEvent event) {
        ...
        if (isValidate) {
            ...
            try {
                boolean isSaved = equipmentBo.saveEquipment(dto);
    ...
    void txtEquipmentIdOnAction(ActionEvent event) {
        ...
        try {
            EquipmentDto equipmentDto = equipmentBo.searchEquipment(equipmentId);
    ...
    private boolean validateEquipment() {
        ...
        boolean matches1=Pattern.matches("[A-Za-z]{5,}",txtEquName.getText());
        if (!matches1) {
