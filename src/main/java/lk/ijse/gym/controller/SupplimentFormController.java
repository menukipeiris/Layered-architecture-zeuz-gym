...
import lk.ijse.gym.dao.Custom.Impl.SupplimentDaoImpl;
...
public class SupplimentFormController {
   ...
   // SupplimentDao supplimentDao = (SupplimentDao) DAOFactory.getDaoFactory().getDao(DAOFactory.DAOTypes.SUPPLIMENT);
    ...
    private void loadAllItems() {
        var model=new SupplimentDaoImpl();
        ObservableList<SupplimentTm> obList = FXCollections.observableArrayList();
        try {
            List<SupplimentDto> dtoList = model.loadAllItem();
    ...
    void btnSaveOnAction(ActionEvent event) {
        ...
        if (isValidate) {
            ...
            try {
                boolean isSaved = supplimentBo.saveItems(dto);
                if (isSaved) {
    ...
    void btnUpdateOnAction(ActionEvent event) {
        ...
        try {
            boolean isUpdated = supplimentBo.updateItems(new SupplimentDto(code, description, unitPrice, qtyOnHand));
            if (isUpdated) {
    ...
    void searchItemOnAction(ActionEvent event) {
        ...
        try {
            SupplimentDto dto = supplimentBo.searchItems(code);
            if (dto != null) {
                setFields(dto);
            } else {
    ...
    public void btnDeleteOnAction(ActionEvent event) {
        String code=txtItemCode.getText();
        try {
            boolean isDeleted = supplimentBo.deleteItems(code);
            if (isDeleted) {
                new Alert(Alert.AlertType.CONFIRMATION, "Suppliment deleted!").show();
            } else {
                new Alert(Alert.AlertType.CONFIRMATION, "Suppliment not deleted!").show();
            }
        } catch (SQLException e){
            new Alert(Alert.AlertType.ERROR, e.getMessage()).show();
        }
