...
import lk.ijse.gym.dao.Custom.Impl.MemberDaoImpl;
...
public class MemberFormController {
    ...
    private void loadAllMembers() {
        var model = new MemberDaoImpl();

        ObservableList<MemberTm> obList = FXCollections.observableArrayList();

        try {
            List<MemberTm> dtoList = model.getAll();
            for (MemberTm dto : dtoList) {
                obList.add(
                        new MemberTm(
                                dto.getMemberId(),
                                dto.getName(),
   ...
   /* private String[]listGender={"Male","Female","Other"};
    ...
    void txtSearchIdOnAction(ActionEvent event) {
        ...
        try {
            MemberDto memberDto =  memberBo.searchMember(memberId);
