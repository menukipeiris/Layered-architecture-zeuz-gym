...
import lk.ijse.gym.dao.Custom.Impl.WorkoutDaoImpl;
...
public class WorkoutFormController {
    ...
    private void loadAllPlans() {
        var model=new WorkoutDaoImpl();

        ObservableList<WorkoutTm> Oblist= FXCollections.observableArrayList();

        try{
            List<WorkoutTm> dtoList= model.getAll();
            for(WorkoutTm dto:dtoList){
                Oblist.add(
                        new WorkoutTm(
                                dto.getPlanId(),
                                dto.getWarmUp(),
    ...
    void btnDeleteOnAction(ActionEvent event) {
        ...
        try {
            boolean isDeleted = workoutBo.deleteService(planId);
            if (isDeleted) {
    ...
    void btnSaveOnAction(ActionEvent event) {
        ...
        if (isValidate) {
            ...
            try {
                boolean isSaved = workoutBo.saveService(dto);
        ...
        void btnUpdateOnAction (ActionEvent event){
            ...
            try {
                boolean isUpdated =workoutBo.updateService(dto);
                if (isUpdated) {
        ...
        void txtSearchWorkoutOnAction (ActionEvent event){
            ...
            try {
                WorkoutDto workoutDto = workoutBo.searchService(planId);
                if (workoutDto != null) {
        ...
        private boolean validateWorkout () {
            ...
            boolean matches1=Pattern.matches("[A-Za-z]{2,}",txtWarmUp.getText());
            if (!matches1) {
                new Alert(Alert.AlertType.ERROR, "Invalid warm up").show();
                return false;
            }
            boolean matches2=Pattern.matches("[A-Za-z]{2,}",txtTraining.getText());
            if (!matches2) {
                new Alert(Alert.AlertType.ERROR, "Invalid training").show();
                return false;
            }
            boolean matches3=Pattern.matches("[A-Za-z]{2,}",txtCardio.getText());
            if (!matches3) {
                new Alert(Alert.AlertType.ERROR, "Invalid class id").show();
                return false;
            }
            boolean matches4=Pattern.matches("[A-Za-z]{2,}",txtWarmDown.getText());
            if (!matches4) {
